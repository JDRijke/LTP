...
def main():
    ...
    def tokenize_function(examples):
        # Using 'text_base' as input and 'fallacy_exists' as label
        return tokenizer(examples['text_base'], truncation=True, padding=True)

    # Map labels to 'label' column as expected by Trainer
    def rename_label(examples):
        examples['label'] = examples['fallacy_exists']
        return examples

    tokenized_datasets = raw_datasets.map(tokenize_function, batched=True)
    tokenized_datasets = tokenized_datasets.map(rename_label)
